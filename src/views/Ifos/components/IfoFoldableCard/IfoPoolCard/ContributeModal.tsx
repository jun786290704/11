...
const ContributeModal: React.FC<Props> = ({
  ...
  const label = currency === tokens.cake ? t('Max. PCM entry') : t('Max. token entry')
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    poolId === PoolIds.poolBasic
      ? t(
          'For the basic sale, Max PCM entry is capped by minimum between your average PCM balance in the IFO PCM pool, or the pool’s hard cap. To increase the max entry, Stake more PCM into the IFO PCM pool',
        )
      : t(
          'For the unlimited sale, Max PCM entry is capped by your average PCM balance in the IFO PCM pool. To increase the max entry, Stake more PCM into the IFO PCM pool',
        ),
    {},
  )
      ...
      <ModalBody maxWidth="360px">
        ...
        <Box p="2px">
          ...
          <Text color="textSubtle" fontSize="12px" mb="24px">
            {t(
              'If you don’t commit enough PCM, you may not receive any IFO tokens at all and will only receive a full refund of your PCM.',
            )}
            <Link
              fontSize="12px"
              display="inline"
              href="https://docs.pcmswap.info/products/ifo-initial-farm-offering"
              external
            >
              {t('Read more')}
            </Link>
